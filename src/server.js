...
const c = {
  ...
  status(code) {
    if (code < 300) return chalk.hex('#10B981')(code);
    if (code < 400) return chalk.hex('#F59E0B')(code);
    return chalk.hex('#EF4444')(code);
  },
};
...
async function handleRequest(req, res, options) {
  ...
  const ctx     = { params, body: reqBody, query, headers: req.headers };
...
function printBanner(routesFile, options, routeList) {
  const W = 54;
...
export async function startServer(routesFile, options) {
  ...
  if (options.watch) {
    watchFile(resolved, async () => {
      try {
        ...
        console.log(`  ${c.error('✗')}  ${c.dim('Reload failed:')} ${err.message}`);
      }
    });
  }
